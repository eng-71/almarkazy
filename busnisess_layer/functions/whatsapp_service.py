...
"""
WhatsApp Notification Service — WASenderAPI Integration
Sends appointment notifications and messages to patients via WhatsApp.

Documentation: https://wasenderapi.com/api/send-message
Provider: WASenderAPI (Third-party WhatsApp Service)
"""
...
WASENDER_API_KEY = os.environ.get('WASENDER_API_KEY', '479512e976ad0f35b286f09f5f5f45941a50c358fed57a2efce328a425fb496d')
SITE_URL = os.environ.get('SITE_URL', 'https://alteb.almarkazy.com')
...
def _format_phone_for_whatsapp(phone: str) -> str:
    """
    Convert a local Egyptian phone number to E.164 format for WhatsApp.
    E.164 format: +[country code][number]
    Examples:
        '01550010435'  →  '+201550010435'
        '1550010435'   →  '+201550010435'
        '201550010435' →  '+201550010435'
        '+201550010435'→  '+201550010435' (already E.164)
    """
    if not phone:
        return ''
    ...
    # Fallback: assume Egypt country code (20) and add + prefix
    return '+20' + phone.lstrip('20')
...
def send_appointment_whatsapp(
    ...
    """
    Send appointment confirmation via WhatsApp using WASenderAPI.
    ...
    Args:
        ...
        appointment_time: Formatted appointment time string
        auto_lookup_url: Full URL for auto-lookup (e.g., https://alteb.almarkazy.com/patient/auto/TOKEN)
        priority: Optional queue priority (not used by WASenderAPI)
        send_at: Optional ISO 8601 datetime to schedule message (not used by WASenderAPI)
    ...
    if not WASENDER_API_KEY:
        logger.warning("WASENDER_API_KEY not configured — skipping WhatsApp notification")
        return {'success': False, 'error': 'WASenderAPI credentials not configured'}
    ...
    message_text = (
        ...
        f"🕐 الوقت: {appointment_time}\n"
        f"👨‍⚕️ الدكتور: {doctor_name}\n\n"
        f"وعشان راحتك اهم حاجة 🥰 \n  تقدر تتابع دورك ومعاد حضورك لايف من هنا : \n"
        f"{auto_lookup_url}"
    )
    
    # Validate message length
    if len(message_text) > 4096:
        logger.error(f"❌ Message exceeds WASenderAPI limit (4096): {len(message_text)} chars")
        return {'success': False, 'error': 'Message text exceeds maximum length (4096 characters)'}
    ...
    # Build payload using WASenderAPI format (E.164 phone number format)
    payload = {
        "to": formatted_phone,
        "text": message_text
    }
    ...
    try:
        ...
        # Success: 200
        if response.status_code == 200:
            msg_id = response_data.get('data', {}).get('msgId') or response_data.get('msgId')
            jid = response_data.get('data', {}).get('jid') or formatted_phone
            status = response_data.get('data', {}).get('status', 'sent')
            logger.info(f"✅ WhatsApp sent to {formatted_phone} — message_id: {msg_id}, status: {status}")
            return {
                'success': True,
                'message_id': msg_id,
                'status': status,
        ...
        # Error handling for various status codes
            ...
            error_details = response_data.get('error', response_data.get('message', 'Bad request'))
            logger.error(f"❌ WASenderAPI validation error (400): {error_details}")
            return {'success': False, 'error': f'Invalid request: {error_details}'}
        
        elif response.status_code == 401:
            logger.error(f"❌ WASenderAPI authentication failed (401) — check API key")
            return {'success': False, 'error': 'Authentication failed: Invalid API key'}
        
        elif response.status_code == 429:
            logger.error(f"❌ WASenderAPI rate limit exceeded (429) — try again later")
            return {'success': False, 'error': 'Rate limit exceeded: Too many requests'}
        
        elif response.status_code == 500:
            logger.error(f"❌ WASenderAPI server error (500)")
            return {'success': False, 'error': 'Server error: WASenderAPI unavailable'}
        
        else:
            error_msg = response_data.get('error', response_data.get('message', response.text))
            logger.error(f"❌ WASenderAPI error ({response.status_code}): {error_msg}")
            return {'success': False, 'error': f'API error ({response.status_code}): {error_msg}'}
    
    except requests.exceptions.Timeout:
        logger.error(f"❌ WASenderAPI timeout for {formatted_phone}")
        return {'success': False, 'error': 'API request timed out (10 seconds)'}
    
    except requests.exceptions.ConnectionError as e:
        logger.error(f"❌ WASenderAPI connection error: {str(e)}")
        return {'success': False, 'error': f'Connection error: {str(e)}'}
    
    except ValueError as e:
        logger.error(f"❌ Invalid JSON response from WASenderAPI: {str(e)}")
        return {'success': False, 'error': 'Invalid API response'}
    
    except requests.exceptions.RequestException as e:
        logger.error(f"❌ WASenderAPI request failed: {str(e)}")
        return {'success': False, 'error': str(e)}
...
def send_text_message(
    ...
    """
    Send a simple text message via WASenderAPI.
    
    Args:
        patient_phone: Recipient phone number (E.164 format)
        message_text: Message text (max 4096 chars)
        priority: Optional queue priority (not used by WASenderAPI)
        send_at: Optional ISO 8601 datetime to schedule (not used by WASenderAPI)
    
    Returns:
        dict with 'success' and 'message_id' or 'error'
    """
    if not WASENDER_API_KEY:
        logger.warning("WASenderAPI credentials not configured")
        return {'success': False, 'error': 'WASenderAPI credentials not configured'}
    ...
    try:
        ...
        if response.status_code == 200:
            msg_id = response_data.get('data', {}).get('msgId') or response_data.get('msgId')
            status = response_data.get('data', {}).get('status', 'sent')
            logger.info(f"✅ Text message sent to {formatted_phone} — id: {msg_id}, status: {status}")
            return {'success': True, 'message_id': msg_id, 'status': status, 'response': response_data}
        else:
            logger.error(f"❌ API error ({response.status_code}): {response_data}")
            return {'success': False, 'error': str(response_data)}
...
def get_api_status() -> dict:
    """
    Verify WASenderAPI connection and credentials.
    ...
    # Test a simple validation by making a test request
    # Note: WASenderAPI doesn't have a dedicated status endpoint,
    # so we verify connectivity through headers validation
    try:
        # Make a HEAD request to check connectivity
        response = requests.head(WASENDER_API_URL, headers=headers, timeout=5)
        
        if response.status_code in [200, 400, 401]:
            logger.info(f"✅ WASenderAPI connectivity verified (status: {response.status_code})")
            return {
                'success': True,
                'status': 'connected',
                'api_url': WASENDER_API_URL,
                'http_status': response.status_code
            }
        else:
            logger.error(f"❌ API connectivity check failed: {response.status_code}")
            return {'success': False, 'error': f'Unexpected API response: {response.status_code}'}
